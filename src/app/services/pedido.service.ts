...
export class PedidoService {
  ...
  crearFactura(idPedido: number): Observable<string> {
    const url = `${this.url}/factura/${idPedido}`;
    return this.http.post<{ url: string }>(url, {}).pipe(
      switchMap(res =>
        this.http.get(res.url, { responseType: 'blob' }).pipe(
          map(res => {
  ...
  crearNotaVenta(idPedido: number): Observable<string> {
    const url = `${this.url}-nota-venta/${idPedido}`;
  ...
  cancelarPedido(idPedido: number): Observable<{ mensaje: string }> {
    const url = `${this.url}/${idPedido}`;
  ...
  obtenerDetallesPedido(
    idPedido: number
  ): Observable<{ detallesPedido: DetallePedido[]; total: number }> {
    const url = `${this.url}/detallesPedido/${idPedido}`;
    return this.http.get<{ detallesPedido: DetallePedido[]; total: number }>(
      url
    );
  }
}
